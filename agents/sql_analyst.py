...
def curate_ques(state: AgentSchema) -> AgentSchema: 
    ...
    response = llm.invoke(f"Curate the following question: {user_question}").text
...
def generate_sql(state: AgentSchema) -> AgentSchema:
    ...
    generated_sql_query = llm.invoke(prompt).text  # Generate the SQL query using the LLM

    # The model likes to wrap the query in a markdown code fence, strip it off
    generated_sql_query = generated_sql_query.strip().strip('`').strip().lstrip('sql').strip()
...
if __name__ == "__main__":
    ...
    print("********************************")
    print(sql_analyst_response['final_answer'])  # Print the final answer
